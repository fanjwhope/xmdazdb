...
<script type="text/javascript">
      ...
      function  getFormData(){
    	  $.ajax({
    		  type:'post',
    		  url:'<%=cxt%>/do/ArchiveDepartDo/getListForZDB?zdbdbName=<%=zdbdbName%>&archiveTypeZDB=<%=archiveTypeZDB%>&archiveType=<%=archiveType%>&lsh=<%=lsh%>',
    		  dataType:'json',
    			   ...
    			   if(data!=null){
    				  ...
    				  for(var i=0;i<data.length;i++){
    			    		...
    			    		str='<tr id="del"><td class="readTb" width="64" align=center><input type='+firstInput+' name=flag id=flag value='+data[i].sortNum+'>&nbsp;</td>';
           					if(data[i].sortNum){sortNumber=data[i].sortNum;}else{ sortNumber=i+1;}
      ...
      function rowContent(num,flag){//插入的时候
		  ...
		  if(flag==1){num=num+1;}
		  str+='<td class=readTb width="330"><input  name=damc id=damc'+num+' size=48 ></td>';
		  str+='<td class=readTb width="64"><input size=8 name=days id=days ></td>';
		  str+='<td class=readTb width="207"><input size=33 name=dabz id= dabz ></td>';
		  str+='<td class=readTb width="75" align=center nowrap></td></tr>'; 
	     return str;
      }
		...
		<center>
			<form name='directoryForm' id="directoryForm" method=post>
				...
				<div
					...
					<table border="3" bordercolorlight="#FFFFFF"
						bordercolordark="#333399" cellspacing="0" cellpadding="0"
						width="770" id="shutaGrid">
						<tr>
						...
						<tr>
							<td colspan='7' align='center' width=100%><font
								style='font-size:10pt' color='blue'>添加的行数<input size='2'
									...
									onclick="cancelZl()"> 打印的起始序号为:<input size='2'
									name='startNum' id='startNum' value='1'
									onkeyup="(this.v=function(){this.value=this.value.replace(/[^0-9]+/,'');}).call(this)"
									onblur="this.v();"> <input type="button" class="button"
									value="卷内文件" name="PrnDhgl" id="PrnDhgl"
									...
									class="button" type="hidden" id="upload" value="上传扫描件"
									name="uploadF" onclick="uploaded()" /> <input type="hidden"
