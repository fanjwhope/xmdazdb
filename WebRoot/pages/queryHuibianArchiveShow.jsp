...
<html>
  <head>
    ...
    function addStdNr(){
        	...
        	var uri;
        	if('<%=yjflag%>'=='2'||"1"=='<%=type%>'){
        		$('#load').attr('src',"<%=cxt%>/pages/achiveDirectoryShow.jsp?zdbdbName=<%=zdbdbName%>&archiveType="+archiveType+"&lsh="+lsh+"&tbname=<%=tbname%>");
        	}else{
           	 $.ajax({
           		 type:'post',
           	     url:'<%=cxt%>/do/ArchiveDo/isNotStand?zdbdbName=<%=zdbdbName%>&archiveType='+archiveType+'&lsh='+'lsh&tbname=<%=tbname%>',
           	     dataType:'json',
           	     success:function(data){
           	    	 if(data.param==1){//1表示非标准
           	    		$('#load').attr('src',"<%=cxt%>/pages/achiveDirectory.jsp?zdbdbName=<%=zdbdbName%>&archiveType="+archiveType+"&lsh="+lsh);
           	    	 }else{
           	    		$('#load').attr('src',"<%=cxt%>/pages/achiveDirectoryStand.jsp?zdbdbName=<%=zdbdbName%>&archiveType="+archiveType+"&lsh="+lsh);
           	    	 }
           	       }
             	});
        	}
         ...
         <hr>
            <tr>
              <td  align='middle'>
                     ...
                     <input type='button' name='button'  class="button" value='选 择' onclick="addStdNr()">&nbsp;<input type="button"  class="button"  value="关 闭" name="okprev" id="okprev"  onclick="clo()">			    
				</td>
            </tr>
          </table>
</td>
