...
String userTabName=(dbName+"_xmwj_"+deptNum).toLowerCase();
%>
...
<html>
  <head>
    ...
    <script type="text/javascript">
        ...
        function _search(){
        	...
        	$('#listPage').show();
        	$('#xmwjList').datagrid({
      			url:"<%=cxt%>/do/PrivilegeInfo/getWebSearch?zdbdbName=<%=zdbdbName%>&dkqx="+$('#dkqx').val()+"&xmbh="+$('#xmbh').val()+"&xmmc="+$('#xmmc').val()+"&qymc="+$('#qymc').val()+"&ywhc="+$('#ywhc').val()+"&by1="+$('#by1').val()+"&projectType="+$('#projectType').combobox('getValue')+"&xmfzr="+$('#xmfzr').val()+"&by2="+$('#by2').val()+"&dkje="+$('#dkje').val()+"&tbDate="+$('#tbDate').datebox('getValue')+"&tbDateE="+$('#tbDateE').datebox('getValue')+"&square="+$('#square').combobox('getValue')+"&roles=<%=roles%>",
     			rownumbers:true,
        ...
        function showForm(lsh,xmbh,yjflag,tbname){
        	if('zdb'=='<%=zdbdbName%>'){
        		$('#load').attr('src','pages/queryHuibianArchiveShowZDB.jsp?zdbdbName=<%=zdbdbName%>&lsh='+lsh+'&xmbh='+xmbh+'&yjflag='+yjflag+'&type=1&tbname='+tbname+'');
        	}else{
        	    $('#load').attr('src','pages/queryHuibianArchiveShow.jsp?zdbdbName=<%=zdbdbName%>&lsh='+lsh+'&xmbh='+xmbh+'&yjflag='+yjflag+'&type=1&tbname='+tbname+'');
        	}
        	$('#listPage').hide();
    		$('#modifyPage').show(); 
        }
  ...
  <body class="easyui-layout" data-options="fit:true">
     <div data-options="region:'center',border:false" style="overflow:hidden">
		<!-- <div id="listPage" style="width:100%;height:100%;padding:0px;margin:0px;display:none;">
		<div id="lp2" class="easyui-layout" data-options="fit:true,border:false">
		    <div data-options="region:'north',border:false,title:'项目文档信息'" style="height:18px;overflow: hidden;" align="left">
		     </div>
		     <div data-options="region:'north'" style="height:20px;overflow: hidden;background-color:ButtonFace "><font style='font-size:11pt'  color="#000080">&nbsp;项目档案查询结果：</font></div>
		     <div data-options="region:'center',border:false" style="width:100%;height:100%;padding:0px;margin:0px;overflow:hidden;">
						  ...
						  <form action='' method="post">
							<table border="0" width="100%" height="131">
							 ...
							 <tr>
							   ...
							   <tr>
							    <td  class="font14" colspan="2">填表时间 <input name="tbDate" class="easyui-datebox" data-options = "editable:false" id="tbDate" type="text" style="width:90px;" /> 至 <input name="tbDateE" id="tbDateE" class="easyui-datebox" data-options = "editable:false" type="text" style="width:90px;" />
							   &#12288;&#12288;&nbsp;是否结清 <input name="square" id="square" type="text" style="width:62px;" /></td>
							   </tr> 
							     <tr>
							    <input name="by2" id="by2" type="hidden" style="width: 136px" />
