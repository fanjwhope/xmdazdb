 ...
 $(function(){
  ...
  function change(operator){
      ...
      switch(operator){
         ...
         case 'pri':
            ...
            $.ajax({
              url:"<%=cxt%>/do/BasicRateDo/up?zdbdbName=<%=zdbdbName%>&id=" + tep,
              type:"post",
              success:function(){
                load(tep);
              }
         ...
         case 'next':
            addDiv.style.display = "none";
            $.ajax({
              url:"<%=cxt%>/do/BasicRateDo/down?zdbdbName=<%=zdbdbName%>&id=" + tep,
              type:"post",
              success:function(){
                load(tep);
              }
         ...
         case 'del':
            if(confirm("确认删除？")){
               $.ajax({
                  url:"<%=cxt%>/do/BasicRateDo/del?zdbdbName=<%=zdbdbName%>&id=" + tep,
                  type:"post",
                  success:function(){
                      load(0);
                      addDiv.style.display = "none";
  ...
  function insertData(){
      ...
      if(cname == "")
     	 ...
     	 $.ajax({
        	 url:"<%=cxt%>/do/BasicRateDo/add?zdbdbName=<%=zdbdbName%>",
        	 data:{'cname':cname},
        	 type:"post",
        	 success:function(){
            	 load(0);
  ...
  function updateData(){
      ...
      if(cname == "")
     	 ...
     	 $.ajax({
        	 url:"<%=cxt%>/do/BasicRateDo/update?zdbdbName=<%=zdbdbName%>",
